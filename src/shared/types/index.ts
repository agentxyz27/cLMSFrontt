...
export type TemplateType = 'DRAG_MATCH' | 'MULTIPLE_CHOICE'
...
// Source of truth for interaction correctness and structure.
// canvas field is generated by the builder, then optionally decorated by teacher..



export type DragMatchSkin = string

export interface DragMatchContent {
  prompt: string
  hints?: string[]
  items: { id: string; label: string; [key: string]: unknown }[]
  targets: { id: string; label: string; accepts: string; [key: string]: unknown }[]
  canvas?: CanvasData
  skin?: DragMatchSkin
...
export type QuestionContentMap = {
  DRAG_MATCH: DragMatchContent
  MULTIPLE_CHOICE: MultipleChoiceContent
}

export type QuestionContent = DragMatchContent | MultipleChoiceContent
